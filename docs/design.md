...
| §2    | "The browser driver … is an _optional peer dependency_, loaded lazily"                                           | Neither a dependency nor a peer dependency; **injected** by the caller (`playwrightDriver(playwright)`), never imported by this package                                     | A lazy `import("playwright")` has no spelling that works for both registries — a bare specifier is unmapped in Deno, `npm:playwright` is invalid in the npm build, and a computed specifier evades both toolchains' analysis. Injection also keeps the driver interface structural, hence testable with no browser. See [plan 04](./plan/04-browser-adapter.md) #1. |
...
| §4    | `bytes()` "if retained (see `retainBody`)" — but `FetchRequest` has no such field                                | `retainBody` is a first-class request option, paired with `hasBody` on the result and four named `BodyAbsentReason`s                                                        | The sketch referenced an option it never declared. Reading an absent body now fails with one kind (`no-body`) plus a `reason`, so all four causes — `retain-body`, `skip-body`, `head`, `not-modified` — stay regular. See [plan 01](./plan/01-public-contracts.md) #6.                                                                                             |
| §5.1  | "Cookie jar: optional, injected, off by default"                                                                 | Not in v1 — pass a `Cookie` header (which is dropped on a cross-origin hop)                                                                                                 | The sketch's own §11 question 3 recommends leaving it out; a correct jar is a package of its own, and nothing in the crawler needs one yet.                                                                                                                                                                                                                         |
| §5.2  | Driver behind "a tiny internal driver interface (`launch`, `newContext`, `newPage`, `goto`, `content`, `close`)" | A larger structural `BrowserDriver`: request filtering, three wait operations, crash and capture hooks, `applyPageOptions`, `raw` handles                                   | The six-method sketch cannot express resource blocking, the wait strategies §5.2 itself requires, or crash detection. It is still smaller than either real driver — everything a page fetcher never does is left out. See [plan 04](./plan/04-browser-adapter.md) #1.                                                                                               |
| §5.2  | `"networkidle"` as a wait condition                                                                              | A **soft hybrid**: load → bounded idle wait → proceed anyway and set `extra.networkidleTimedOut` (`strict: true` restores hard failure)                                     | Pages carrying analytics or a websocket never go quiet at all; a hard-failing networkidle makes the adapter unusable on exactly the sites it exists for. The explicit conditions (`{ selector }`, `{ fn }`) do still fail hard — the caller named a condition. See [plan 04](./plan/04-browser-adapter.md) #4.                                                      |
| §5.2  | Context strategy `"per-request"`                                                                                 | Still capped at `poolSize` concurrent contexts                                                                                                                              | An uncapped mode is a footgun, not a feature: one burst of concurrency would launch contexts until the browser died. See [plan 04](./plan/04-browser-adapter.md) #3.                                                                                                                                                                                                |
...
| §6    | Breaker counts failures and successes                                                                            | Three-way: failure / success / **inconclusive**                                                                                                                             | `aborted`, `deadline` and a nested `circuit-open` prove nothing about the host, so they neither count nor reset — and an aborted probe releases its slot without opening or closing the circuit. Treating "not a failure" as "healthy" would let a cancelled probe close a circuit on a host that is still down.                                                    |
...
| §7    | "A descriptive default User-Agent that identifies the tool"                                                      | HTTP adapter only. The browser adapter sets **no** `User-Agent` by default                                                                                                  | Replacing a real browser's UA with a bot string is what gets a headless browser served different HTML or blocked outright — which defeats the only reason to run one. `userAgent` is one option away.                                                                                                                                                               |
| §8    | Dev cache "keyed by method+URL+adapter"                                                                          | GET only in v1; every other method bypasses the layer (the method stays in the key string)                                                                                  | A cached HEAD entry would be bodyless, breaking the `CachedEntry` invariant, for ~zero savings. Keeping the method in the key means widening this later needs no migration. See [plan 05](./plan/05-cache-layer.md) #2.                                                                                                                                             |
...
| §9    | `onRequest(req)`                                                                                                 | `onRequest(req, { requestId, attempt })`                                                                                                                                    | Per-attempt granularity is unusable without knowing which attempt fired it.                                                                                                                                                                                                                                                                                         |
| §9    | "The package emits, it does not log"                                                                             | Emits **and** offers an optional `logger`, silent unless one is injected                                                                                                    | Two complementary channels: `events` is the machine one with defined granularity, `logger` the human one. The package still never writes to the console on its own. See [plan 01](./plan/01-public-contracts.md) #1.                                                                                                                                                |
| §10   | "Assert no child processes remain"                                                                               | A best-effort `ps` scan of `Deno.pid`'s transitive children, inside the flagged browser suite, skipped off darwin/linux — and it asserts the scan **finds** a browser first | Neither driver exposes a portable process handle (Playwright's `Browser` has none). Without the positive assertion first, a broken `ps` parse or a driver that never launched would make the post-dispose check pass for the wrong reason. See [plan 06](./plan/06-testing-docs-tooling.md) #5.                                                                     |
...
## §11's open questions, as resolved

| # | Question                                           | Resolution                                                                                                                                                                                        |
| - | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 | Is `text()` lazy or eager?                         | **Neither branch of the sketch's own suggestion.** Bytes are eager for every adapter, the bytes→string decode is lazy and memoized — one behavior, not one per adapter (see the §11.1 row above). |
| 2 | HTTP/2 and keep-alive tuning?                      | **Deferred to the platform**, as suggested. No knob is exposed; `fetch` is injectable if you need to bring your own.                                                                              |
| 3 | Bundle a cookie jar?                               | **Left out of v1**, as suggested. Pass a `Cookie` header; it is dropped on cross-origin hops.                                                                                                     |
| 4 | A `HEAD`-then-`GET` helper here or in the crawler? | **Not here.** It is a two-request policy, and this package's unit is one request. The README shows the escalation recipe instead.                                                                 |
